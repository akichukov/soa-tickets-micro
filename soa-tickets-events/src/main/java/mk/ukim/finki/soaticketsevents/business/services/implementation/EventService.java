package mk.ukim.finki.soaticketsevents.business.services.implementation;
...
import mk.ukim.finki.soaticketsevents.repositories.IEventRepository;
...
public class  EventService implements IEventService {
    ...
    @Override
    public Long update(UpdatesEventViewModel event) throws Exception {
        DateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy");
        Date eventDate = dateFormat.parse(event.getDate());
        if (eventDate.compareTo(new Date()) < 0)
            throw new Exception("The event can't be in the past.");
